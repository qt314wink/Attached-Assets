...
export function BrutalistHero({ setPage }: { setPage: (p: string) => void }) {
  ...
  const modules = [
    { title: "Generative Engine", icon: <Zap size={32} />, color: "bg-[#FF3366]", x: 5, y: 15, w: "w-72", id: "module-somatic" },
    { title: "Collage Pipeline", icon: <Sparkles size={32} />, color: "bg-[#33CCFF]", x: 45, y: 5, w: "w-80", id: "module-materials" },
    { title: "Asset Database", icon: <Database size={32} />, color: "bg-[#CCFF00]", x: 25, y: 45, w: "w-64", id: "module-liquid" },
    { title: "Component UI", icon: <Layers size={32} />, color: "bg-white", x: 65, y: 35, w: "w-72", id: "module-neumorphic" },
  ];
    ...
    <div className="h-screen min-h-[800px] bg-[#E5E5E5] overflow-hidden relative font-sans" ref={constraintsRef}>
        ...
        >
          ...
          <div className="bg-white p-4 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            ...
            <button 
              ...
              className="bg-black text-white font-bold uppercase tracking-widest text-xs py-2 px-4 hover:bg-[#CCFF00] hover:text-black border-2 border-black transition-colors"
            >
              Access Module
            </button>
          </div>
      ...
      <div className="absolute bottom-12 right-12 bg-white border-8 border-black p-10 shadow-[24px_24px_0px_0px_rgba(0,0,0,1)] max-w-xl z-20 hidden md:block">
        ...
        <div className="flex gap-4">
          <button onClick={() => setPage('systems')} className="flex-1 bg-[#CCFF00] hover:bg-black hover:text-white text-black font-black uppercase tracking-widest py-4 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] active:translate-x-2 active:translate-y-2 active:shadow-[0px_0px_0px_0px_rgba(0,0,0,1)] transition-all text-sm">
            Explore Components
          </button>
          <button onClick={() => setPage('forge')} className="flex-1 bg-white hover:bg-black hover:text-white text-black font-black uppercase tracking-widest py-4 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] active:translate-x-2 active:translate-y-2 active:shadow-[0px_0px_0px_0px_rgba(0,0,0,1)] transition-all text-sm">
            Access Forge
          </button>
        </div>
      </div>
